...
import com.rahman.aplikasihitungkata.network.ImageApi
...
class ImageViewModel : ViewModel() {
    ...
    fun uploadImage(userEmail: String,nama: String?, genre: String?, rilis: String?, bitmap: Bitmap) {
        viewModelScope.launch(Dispatchers.IO) {
            apiStatus.value = ApiStatus.LOADING
            errorMessage.value = null
            try {
                val namaRequestBody = nama?.takeIf { it.isNotBlank() }?.toRequestBody("text/plain".toMediaTypeOrNull())
                val genreRequestBody = genre?.takeIf { it.isNotBlank() }?.toRequestBody("text/plain".toMediaTypeOrNull())
                val rilisRequestBody = rilis?.takeIf { it.isNotBlank() }?.toRequestBody("text/plain".toMediaTypeOrNull())
                val imagePart = bitmap.toMultipartBody()

                val result = ImageApi.service.uploadImage(
                    userEmail = userEmail,
                    nama = namaRequestBody,
                    genre = genreRequestBody,
                    rilis = rilisRequestBody,
                    gambar = imagePart
                )
    ...
    fun updateImage(
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                val genreBody = newGenre?.toRequestBody("text/plain".toMediaTypeOrNull())
                val rilisBody = newRilis?.toRequestBody("text/plain".toMediaTypeOrNull())
